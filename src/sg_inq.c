...
static const char * version_str = "2.70 20260921";  /* spc7r05, sbc6r02 */
    ...
    if (len <= 8) {
        ...
        if (len <= 16) {
            ...
            if (op->do_export) {
                ...
                if (mlen > 0) {
                    printf("SCSI_MODEL=");
                    /* escape '/' so it cannot be taken as a path separator
                     * when the value is used to build a /dev/disk/by-id
                     * symlink name */
                    for (m = 0; m < mlen; ++m) {
                        if ('/' == xtra_buff[m])
                            printf("\\x2f");
